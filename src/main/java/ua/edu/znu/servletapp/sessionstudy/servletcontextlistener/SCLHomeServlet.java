...
@WebServlet("/SCLHomeServlet")
public class SCLHomeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        System.out.println("SCLHomeServlet: sessionID = " + sessionID);

        SCAppSession session = sessions.get(sessionID);
        User user = (User) session.getSessionAttribute("user");
        System.out.println("SCLHomeServlet: user = " + user);
        String message;
        if (user != null) {
